...
def speak(text: str) -> None:
    ...
    if not os.path.exists(f"answers/{today}.csv"):
        open(f"{today}.csv", 'a').close()

    # append current question to the csv
    with open(f"answers/{today}.csv", "a", newline="") as answers:
...
def listen(OVERRIDE=False) -> None:
    ...
    try:
        ...
        audio_file_path = os.path.join(os.path.dirname(__file__), 'audioFile.wav')
        ...
        transcribe_response = whisperModel.transcribe(audio_file_path)
        ...
        # check and make todays csv
        if not os.path.exists(f"questions/{today}.csv"):
            open(f"{today}.csv", 'a').close()

        # append current question to the csv
        with open(f"questions/{today}.csv", "a", newline="") as questions:
            # creating writer object
